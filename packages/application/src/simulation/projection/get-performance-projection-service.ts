...
export interface GetPerformanceProjectionInput {
...
export interface GetPerformanceProjectionResult {
  readonly projection: PerformanceProjection;
  readonly freshness: ProjectionFreshness;
}

export interface GetPerformanceProjectionServiceDeps {
  ...
  readonly rebuildService: RebuildPerformanceProjectionService;
}

export interface GetPerformanceProjectionService {
  get(
    input: GetPerformanceProjectionInput,
  ): Promise<Result<GetPerformanceProjectionResult, CommandError>>;
}

export const createGetPerformanceProjectionService = (
  deps: GetPerformanceProjectionServiceDeps,
  ...
  async get(input) {
    ...
    const cached =
      cachedRow && isPerformanceProjection(cachedRow) ? cachedRow : null;
      ...
      if (relation === "newer") {
        return err(
          ruleViolationError(
            ...
            "Cached Performance projection source is ahead of the authoritative SimulationRun.",
          ),
        );
      }
    }
