...
export interface GetCoachingProjectionInput {
...
export interface GetCoachingProjectionResult {
  readonly projection: CoachingProjection;
  readonly freshness: ProjectionFreshness;
}

export interface GetCoachingProjectionServiceDeps {
  ...
  readonly rebuildService: RebuildCoachingProjectionService;
}

export interface GetCoachingProjectionService {
  get(
    input: GetCoachingProjectionInput,
  ): Promise<Result<GetCoachingProjectionResult, CommandError>>;
}

export const createGetCoachingProjectionService = (
  deps: GetCoachingProjectionServiceDeps,
  ...
  async get(input) {
    ...
    const cached =
      cachedRow && isCoachingProjection(cachedRow) ? cachedRow : null;
      ...
      if (relation === "newer") {
        return err(
          ruleViolationError(
            ...
            "Cached Coaching projection source is ahead of the authoritative SimulationRun.",
          ),
        );
      }
    }
