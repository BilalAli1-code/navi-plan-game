...
export interface GetMeetingsProjectionInput {
...
export interface GetMeetingsProjectionResult {
  readonly projection: MeetingsProjection;
  readonly freshness: ProjectionFreshness;
}

export interface GetMeetingsProjectionServiceDeps {
  ...
  readonly rebuildService: RebuildMeetingsProjectionService;
}

/**
 * Application query: get learner-facing Meetings projection.
 * Binds projection type `meetings` internally. Missing/stale → sync rebuild.
 * Side-effect free for authoritative state (no lifecycle mutation).
 */
export interface GetMeetingsProjectionService {
  get(
    input: GetMeetingsProjectionInput,
  ): Promise<Result<GetMeetingsProjectionResult, CommandError>>;
}

export const createGetMeetingsProjectionService = (
  deps: GetMeetingsProjectionServiceDeps,
  ...
  async get(input) {
    ...
    const cached =
      cachedRow && isMeetingsProjection(cachedRow) ? cachedRow : null;
      ...
      if (relation === "newer") {
        return err(
          ruleViolationError(
            ...
            "Cached Meetings projection source is ahead of the authoritative SimulationRun.",
          ),
        );
      }
    }
