...
export interface GetDecisionLogProjectionInput {
...
export interface GetDecisionLogProjectionResult {
  readonly projection: DecisionLogProjection;
  readonly freshness: ProjectionFreshness;
}

export interface GetDecisionLogProjectionServiceDeps {
  ...
  readonly rebuildService: RebuildDecisionLogProjectionService;
}

/**
 * Application query: get learner-facing Decision Log projection.
 * Binds projection type `decision_log` internally. Missing/stale → sync rebuild.
 */
export interface GetDecisionLogProjectionService {
  get(
    input: GetDecisionLogProjectionInput,
  ): Promise<Result<GetDecisionLogProjectionResult, CommandError>>;
}

export const createGetDecisionLogProjectionService = (
  deps: GetDecisionLogProjectionServiceDeps,
  ...
  async get(input) {
    ...
    const cached =
      cachedRow && isDecisionLogProjection(cachedRow) ? cachedRow : null;
      ...
      if (relation === "newer") {
        return err(
          ruleViolationError(
            ...
            "Cached Decision Log projection source is ahead of the authoritative SimulationRun.",
          ),
        );
      }
    }
