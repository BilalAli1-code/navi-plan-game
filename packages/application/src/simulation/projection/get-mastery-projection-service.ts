...
export interface GetMasteryProjectionInput {
...
export interface GetMasteryProjectionResult {
  readonly projection: MasteryProjection;
  readonly freshness: ProjectionFreshness;
}

export interface GetMasteryProjectionServiceDeps {
  ...
  readonly rebuildService: RebuildMasteryProjectionService;
}

export interface GetMasteryProjectionService {
  get(
    input: GetMasteryProjectionInput,
  ): Promise<Result<GetMasteryProjectionResult, CommandError>>;
}

export const createGetMasteryProjectionService = (
  deps: GetMasteryProjectionServiceDeps,
  ...
  async get(input) {
    ...
    const cached =
      cachedRow && isMasteryProjection(cachedRow) ? cachedRow : null;
      ...
      if (relation === "newer") {
        return err(
          ruleViolationError(
            ...
            "Cached Mastery projection source is ahead of the authoritative SimulationRun.",
          ),
        );
      }
    }
